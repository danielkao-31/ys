...
const GAS_URL = __ENV.GAS_URL || 'https://script.google.com/macros/s/AKfycbzKqPnHLbQz0n3UtuqOx46aNeMaMyVHYWRgzlYr_hiTZ7qEm2UCx2JF2qWl0wybBlEjaQ/exec';
const ASSET_BASE_URL = __ENV.ASSET_BASE_URL || 'https://raw.githubusercontent.com/danielkao-31/ys/main';
...
export function testGithubAssets() {
  const requests = ASSET_PATHS.map((path) => ({
    ...
    params: {
      redirects: 5,
      timeout: '20s',
      tags: { target: 'github_asset', asset: path },
    },
  }));
  ...
  for (const response of responses) {
    const ok = check(response, {
      ...
      'GitHub asset has content': (r) => Number(r.headers['Content-Length'] || r.body?.length || 0) > 0,
    });
...
export function testGasHome() {
  const response = http.get(GAS_URL, {
    redirects: 10,
    timeout: '30s',
    tags: { target: 'gas_home' },
  });

  const ok = check(response, {
    'GAS homepage status is 200': (r) => r.status === 200,
    'GAS homepage returned HTML': (r) => String(r.headers['Content-Type'] || '').includes('text/html'),
    'GAS homepage body is not empty': (r) => Boolean(r.body && r.body.length > 100),
  });
